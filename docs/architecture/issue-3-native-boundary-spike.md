...
Both native inbox scanners fail closed when a manifest is unreadable, malformed, points outside the application-owned Inbox, or claims a copied file that is missing or has a different byte count. iOS also distinguishes a genuinely absent Inbox from an unavailable App Group or unreadable existing Inbox. The TypeScript boundary independently validates schema version, timestamps, MIME types, owned local-file URI shape, non-negative safe-integer byte counts, and finite OCR metrics before exposing a DTO. Valid manifests are ordered by creation time with a stable ingestion-ID tie-breaker. Android normalizes wildcard share types through the content provider, rejects shares without a concrete image MIME or stream, and surfaces failed or malformed outcomes as stable UI error codes. A generation gate prevents older inbox scans from overwriting those failures.
