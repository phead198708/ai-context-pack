...
`npm run audit:ci` reports no critical or unapproved high findings. It admits only the owner-approved, machine-pinned `image-size` ICNS and JXL/HEIF denial-of-service advisories on Metro's build-time asset path; `docs/development/ci.md` records the exact scope, controls, and removal triggers. Moderate transitive Expo/React Native CLI build-tool advisories (`fast-xml-parser` and `uuid` paths) remain visible for dependency-refresh work because no compatible direct upgrade is available without leaving the pinned RN/Expo baseline.
