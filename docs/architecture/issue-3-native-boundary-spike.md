...
| Expo                           | 57.0.11                | Modules API, CLI, autolinking       | SDK 57 supports RN 0.86; maintained by Expo        | MIT                  | Adds local module infrastructure on both platforms                                |
| @expo/cli / metro-config       | 57.0.13 / 57.0.7       | Exact local build / lock entry pins | Maintained SDK 57 tooling; dev / optional lock pin | MIT                  | Consumed through Expo; no app-binary or imported-content runtime path             |
...
`npm run audit:ci` reports no critical, moderate, or unapproved high findings. It admits only the owner-approved, machine-pinned `image-size` ICNS and JXL/HEIF denial-of-service advisories on Metro's build-time asset path; `docs/development/ci.md` records the exact scope, controls, and removal triggers. The React Native CLI 20.2.0 compatible patch removes the former `fast-xml-parser` path, while the xcode-scoped `uuid` 11.1.1 override removes the former UUID advisory without adding a runtime dependency.
