...
files over 50 MiB. JPEG, PNG, and WebP inputs may contain at most 16 million
source pixels; formats that require whole-image fallback are capped at one
million pixels on both platforms. The v1 hasher uses integer box
...
require whole-image fallback retain every source pixel under the shared
one-million-pixel cap and use bounded cancellable source reads. The v0.1 Pack
item cap bounds pairwise candidate comparison.
