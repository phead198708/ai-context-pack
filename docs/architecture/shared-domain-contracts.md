...
PDF inspection creates one task-scoped native source session bound to the lowercase SHA-256 supplied by the owned artifact record. Swift reads a single `O_NOFOLLOW` descriptor in cancellable 64 KiB chunks, enforces the 50 MiB cap while reading, hashes those exact bytes, and retains the resulting immutable `Data`-backed `PDFDocument`. Kotlin applies the same bounded, cancellable hash to a duplicated reader while retaining the original read-only `ParcelFileDescriptor`; every renderer uses a duplicate of that same descriptor and verifies its regular-file identity, size, and modification time. Pages are therefore never re-opened or re-hashed by path, and an atomic path replacement cannot mix another PDF into the inspected document or its checkpoint. The session is explicitly released on success, failure, cancellation, or module destruction.

The embedded-text density threshold uses non-whitespace UTF-16 code units on both platforms. Sparse-text containment also compares raw UTF-16 code units, avoiding Swift canonical-equivalence behavior that Kotlin and JavaScript do not share. When sparse embedded text triggers OCR, the exact embedded text remains in the semantic `text` result: an OCR result that already contains it is reused, otherwise both lossless embedded text and OCR output are retained in deterministic order. New sparse rendered-OCR results also carry optional V1 `embeddedText` provenance; the shared boundary requires it in production and reconstructs `text` from that exact prefix plus the ordered OCR blocks instead of accepting an arbitrary unmatched prefix.
