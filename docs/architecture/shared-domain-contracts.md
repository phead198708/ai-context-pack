...
Every native page-extraction request also carries the lowercase SHA-256 produced by document inspection. Swift and Kotlin verify that digest before opening the page and again before returning its result, so replacing the controlled path cannot mix pages from another PDF into the inspected document or its checkpoint. The embedded-text density threshold uses non-whitespace UTF-16 code units on both platforms. When sparse embedded text triggers OCR, the exact embedded text remains in the semantic `text` result: an OCR result that already contains it is reused, otherwise both lossless embedded text and OCR output are retained in deterministic order.
