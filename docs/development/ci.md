...
| `iOS app & Share Extension` | macOS: shared Jest, workspace-independent podspec checksum regression, media fixture XCTest, native XCTest, unsigned two-target build | `npm test -- --ci && bundle exec ruby -rcocoapods scripts/verify-podspec-checksum.rb && bundle exec pod install --project-directory=ios --deployment && git diff --exit-code -- ios/Podfile.lock ios/AIContextPack.xcodeproj/project.pbxproj ios/AIContextPack/PrivacyInfo.xcprivacy && swift test --package-path modules/context-native/ios && xcodebuild -workspace ios/AIContextPack.xcworkspace -scheme AIContextPack -configuration Debug -destination 'generic/platform=iOS Simulator' CODE_SIGNING_ALLOWED=NO build` |
