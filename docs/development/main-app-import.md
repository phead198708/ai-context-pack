...
5. A stale or revoked provider becomes a visible `IMPORT_PROVIDER_PERMISSION_EXPIRED` item. Unsupported or oversized items are visible failures; successful siblings remain committed in order. For a bounded regular picker file that fails after bytes were readable (for example, a declared-type mismatch), native ingestion stores a separate anonymous `.retry` source inside the same atomic Inbox transaction. Handoff hashes and publishes that source as the failed ContextItem's immutable original; SQLite rehydrates only its controlled relative path, byte count, and SHA-256. **Retry failed items** is shown only when at least one such source exists and creates a new ingestion whose `owned-file` DTO is independently path-, size-, and hash-verified on both platforms. The old Pack keeps its original; retry never reopens a provider URI. Revoked and over-limit sources have no retry target because complete bounded bytes do not exist.
...
- iOS shared writer, stale provider, mixed results, committed failed-item retry across ACK/restart, failed-publish retry retention, content integrity, cache cleanup, and path rejection: `MainAppImportPublisherTests.swift`
