...
ACK runs under the same cross-process registry used to publish writer ownership. Before removing `Inbox/<ingestion-id>`, it validates the exact manifest and atomically publishes and synchronizes `InboxAcknowledgements/<ingestion-id>.json`. The receipt contains only the already-validated, bounded manifest metadata: generated UUIDs, media types, byte counts, hashes, generated relative names, status, and stable error codes. It contains no artifact bytes, provider URI, display filename, imported text, or URL value. Receipts remain in app-private storage for the lifetime of the ingestion ID so a delayed duplicate publisher cannot turn a consumed ID into a new import.

The production manifest reader rechecks item identity, order, path confinement, byte count, SHA-256, aggregate status, and the exact domain error catalog. Replaying the same ingestion ID reads the already-published manifest—or, after ACK, its structurally validated receipt—without reopening the provider. A process death before the final directory rename cannot expose a committed half-manifest; the existing recovery scanner owns abandoned staging cleanup. A process death after receipt publication but before Inbox/tombstone cleanup also remains a replay. A legacy or interrupted ACK tombstone without a valid receipt fails closed until ACK maintenance reconstructs the receipt from an intact, artifact-validated tombstone.
...
- Automated native evidence: Swift Package Manager passed 95 tests. An isolated 20-by-5-MiB ingestion benchmark copied 100 MiB in 215 ms with a reusable 64 KiB input buffer and 32,768 bytes of sampled physical-footprint growth. The benchmark samples current `TASK_VM_INFO.phys_footprint` at ingestion checkpoints; it does not subtract process-lifetime high-water marks, which can conceal growth when earlier tests already raised the baseline. Android passed 43 JVM tests and reported 96 instrumentation tests on `Pixel_9_Pro(AVD) - 15`, with zero failures/errors and one API-conditioned PDF fallback test skipped on API 35. The full Android app/native unit, lint, and Debug build invocation completed 643 Gradle tasks successfully. Both native suites cover a duplicate publisher waiting across ACK, interruption immediately after receipt publication, and recovery of a legacy ACK tombstone without a receipt. The iOS boundary suite also verifies that future ACK receipt and legacy tombstone schemas remain `SCHEMA_VERSION_UNSUPPORTED` instead of being downgraded to storage or schema-invalid errors.
- Shared evidence: format, lint, strict TypeScript, 16 Jest suites/466 tests, 21 schema fixtures, persistence migrations, production persistence, workflow security, the fail-closed npm audit policy suite, and the 20-case v0.1 structured-policy suite passed. The earlier temporary `image-size` exception was superseded by the pinned local `@aicp/image-size` security fork: its complete tree and Metro/queue lock edges are hashed, crafted ICNS/JXL/HEIF payloads are timeout-tested, alternate registry copies fail closed, and `npm run audit:ci` now requires zero findings at every severity with zero exceptions. The workflow verifier continues to reject raw-audit bypasses including global npm options, npm command abbreviations, backslash/standard/ANSI-C quote composition, and unpinned jobs. Expo Doctor passes all checks while the three already-reviewed SDK pins are explicitly excluded from dynamic patch recommendations pending a dedicated dependency issue.
