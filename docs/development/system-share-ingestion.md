...
- Automated native evidence: Swift Package Manager passed 86 tests. The final 20-by-5-MiB ingestion benchmark copied 100 MiB in 351 ms with a 64 KiB stream buffer and 256 KiB measured resident-memory growth during the benchmark. Android passed 43 JVM tests and 85 instrumentation tests on `Pixel_9_Pro(AVD) - 15`, with zero failures/errors and one API-conditioned PDF fallback test skipped on API 35. The combined Android unit/lint/debug-build/instrumentation invocation completed 663 Gradle tasks successfully.
