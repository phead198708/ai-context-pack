...
The declared provider MIME type is only a compatibility hint. Copied bytes must independently detect as a supported type, and a concrete declared type must agree with the detected type. A wildcard or `application/octet-stream` declaration may resolve to any supported detected type. URL query values and imported text never enter diagnostics or logs.
...
5. Synchronize `manifest.partial`, atomically rename it to `manifest.json`, then atomically rename the whole ingestion directory into `Inbox/<ingestion-id>`.
...
`ShareViewController` flattens `NSExtensionItem` attachments in host order and loads one representation at a time. Activation rules expose up to 20 images/files/web URLs plus plain text. The extension only validates, copies, hashes, publishes the manifest, and displays accepted/rejected/failed counts. It does not start React Native, open the containing app, OCR, render PDF pages, compress, detect secrets, redact, or export.

### Android

`MainActivity` accepts `ACTION_SEND` and `ACTION_SEND_MULTIPLE`. ClipData values retain host order, then distinct `EXTRA_STREAM` values are appended in their list order; URIs mirrored in both carriers are represented once, and distinct text follows the URI values. ContentResolver streams are opened only inside the process-global serialized importer while temporary grants are valid. Each ingestion also holds a cross-process writer lock; a caller that times out before acquiring that lock never removes the active writer's staging directory. The Activity replaces its retained launch Intent with `ACTION_MAIN`, and only application-owned Inbox paths survive. A durable share-result event causes the React Native workflow to scan and open the import preview.
...
- Automated native evidence: Swift Package Manager passed 78 tests. The 20-by-5-MiB ingestion benchmark copied 100 MiB in 334 ms with a 64 KiB stream buffer and zero measured resident-memory growth during the benchmark. Android passed 35 JVM tests and 79 instrumentation results on `Pixel_9_Pro(AVD) - 15`, with zero failures/errors and one API-conditioned PDF fallback test skipped on API 35. The combined Android unit/lint/debug-build/instrumentation invocation completed 675 Gradle tasks successfully.
- Shared evidence: format, lint, strict TypeScript, 16 Jest suites/464 tests, 21 schema fixtures, persistence migrations, production persistence, workflow security, and the 20-case v0.1 structured-policy suite passed. `npm audit --audit-level=high` exited zero with no high/critical finding. Expo Doctor passed 18/19 checks; its only failure was the external compatibility metadata expecting `expo ~57.0.11` while the repository intentionally remains pinned to `57.0.10` in this issue.
