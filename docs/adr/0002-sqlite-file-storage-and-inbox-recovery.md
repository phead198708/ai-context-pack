...
- immutable `PRAGMA user_version` migrations from empty → v1 → v2 → v3 → v4 → v5 → v6;
...
publication/cleanup lease is renewed and fenced throughout both critical sections; loss aborts
