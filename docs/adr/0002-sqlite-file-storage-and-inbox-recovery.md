...
- immutable `PRAGMA user_version` migrations from empty → v1 → v2 → v3 → v4 → v5 → v6 → v7 → v8;
...
| v1-v7 app database update           | v8 applied with rows, references, runs, order, release intent, checkpoints, and analyses preserved | `npm run test:persistence-migrations`                         |
| Empty/legacy/restart/concurrent DB  | 0→1→2→3→4→5→6→7→8, ordered restart, one-of-two CAS, rollback, backup/restore pass                  | `npm run test:persistence-production`                         |
